...
// Tab Switching with URL Address Functionality
document.addEventListener('DOMContentLoaded', function() {
    // Get all sidebar menu items
    const sidebarItems = document.querySelectorAll('.sidebar-menu li[data-tab]');
    ...
    function activateTab(tabId, updateHistory = true) {
        ...
        const clickedItem = document.querySelector(`.sidebar-menu li[data-tab="${tabId}"]`);
        if (clickedItem) {
        ...
        // Update URL address without page reload
        if (updateHistory) {
    ...
    function updatePageTitle(tabId) {
        ...
        const baseTitle = 'Kindergarten Management System';
    ...
    function getTabFromURL() {
        ...
        const validTabs = ['instructions', 'students', 'attendance', 'history', 
                          'reports', 'marks', 'leave', 'fees'];
        
        if (hash && validTabs.includes(hash)) {
            return hash;
        }
        return 'instructions'; // Default tab
    }
    
    // Function to check URL hash and activate corresponding tab
    function checkAndActivateTabFromURL() {
        const tabId = getTabFromURL();
        activateTab(tabId, false); // Don't update history (already in URL)
    }
    ...
    // Initial setup - check URL or localStorage
    const urlTab = getTabFromURL();
    const savedTab = localStorage.getItem('activeTab');
    
    // Priority: URL hash > localStorage > default
    let initialTab = 'instructions';
    
    if (urlTab !== 'instructions') {
        initialTab = urlTab;
    ...
    // Also handle direct clicks on browser refresh
    window.addEventListener('load', function() {
        checkAndActivateTabFromURL();
    });
});

// Optional: Make the function globally accessible
window.switchToTab = function(tabId) {
    ...
    document.title = `${tabNames[tabId] || 'Dashboard'} | Kindergarten Management System`;
...
document.addEventListener('keydown', function(e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key >= '1' && e.key <= '8') {
        ...
        if (sidebarItems[index]) {
            const tabId = sidebarItems[index].getAttribute('data-tab');
            window.switchToTab(tabId);
        }
    }
    
    // Escape key to go back to instructions
    if (e.key === 'Escape' && !e.target.matches('input, textarea, select')) {
        window.switchToTab('instructions');
    }
});

// Optional: Add progress indicator for tab switching
...
// Optional: Add tab change animation
function animateTabChange(oldTabId, newTabId) {
    const oldTab = document.getElementById(oldTabId);
    const newTab = document.getElementById(newTabId);
    
    if (oldTab && newTab) {
        // Show loading briefly
        showTabLoading(newTabId);
